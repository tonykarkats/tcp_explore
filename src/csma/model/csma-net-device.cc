...
CsmaNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CsmaNetDevice")
    ...
    .AddAttribute ("ecnThresh", 
                   "Threshold of queue for marking packets with ECN.",
                   UintegerValue (),
                   MakeUintegerAccessor (&CsmaNetDevice::m_ecnThresh),
...
CsmaNetDevice::TransmitReadyEvent (void)
{
  ...
  if (m_queue->IsEmpty ())
  ...
  else
    {
      ...
      m_currentPkt = item->GetPacket ();
	  //std::cout << "DEQUED PACKET: " << m_currentPkt->GetUid() <<" \n";
      m_snifferTrace (m_currentPkt);
      m_promiscSnifferTrace (m_currentPkt);
      TransmitStart ();
    }
...
CsmaNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  AddHeader (packetCopy, source, destination, protocolNumber);

  m_macTxTrace (packetCopy);
