...
#include "path-hashing-helper.h"
#include "ns3/log.h"
#include "ns3/path-hashing-net-device.h"
#include "ns3/node.h"
#include "ns3/names.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("PathHashingHelper");

PathHashingHelper::PathHashingHelper ()
{
  NS_LOG_FUNCTION_NOARGS ();
  m_deviceFactory.SetTypeId ("ns3::PathHashingNetDevice");
}

void
PathHashingHelper::SetDeviceAttribute (std::string n1, const AttributeValue &v1)
{
  NS_LOG_FUNCTION_NOARGS ();
...
NetDeviceContainer
PathHashingHelper::Install (Ptr<Node> node, SwitchType st, NetDeviceContainer uplinkDevices,
                              ...
                              Ipv4Mask mask, int totalPaths,  double timeWindow)
{
  ...
  Ptr<PathHashingNetDevice> dev = m_deviceFactory.Create<PathHashingNetDevice> ();
  devs.Add (dev);
  node->AddDevice (dev);
  ...
  if (st == SwitchType::EDGE_SWITCH) {
    ...
    dev->totalPackets = 0;
    Simulator::Schedule(Seconds(dev->timeWindow), &PathHashingNetDevice::resetWindow, dev);
  }
...
NetDeviceContainer
PathHashingHelper::Install (std::string nodeName, SwitchType st, NetDeviceContainer uplinkDevices,
                              std::map<uint32_t, Ptr<NetDevice>> addressToInterface,
                              Ipv4Mask mask, int totalPaths, double timeWindow)
{
  NS_LOG_FUNCTION_NOARGS ();
