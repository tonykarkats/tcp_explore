 ...
 */
#ifndef PATH_HASHING_NET_DEVICE_H
#define PATH_HASHING_NET_DEVICE_H

#include "ns3/net-device.h"
#include "ns3/mac48-address.h"
#include "ns3/nstime.h"
#include "ns3/path-hashing-channel.h"
#include <stdint.h>
#include <string>
#include <map>
#include "ns3/path-hashing-helper.h"
 ...
 *
 * The PathHashingNetDevice object is a "virtual" netdevice that aggregates
 * multiple "real" netdevices and implements the data plane forwarding
 * part of IEEE 802.1D.  By adding a PathHashingNetDevice to a Node, it
 ...
 * \brief a virtual net device that bridges multiple LAN segments
 */
class PathHashingNetDevice : public NetDevice
{
public:
  /**
  ...
  PathHashingNetDevice ();
  virtual ~PathHashingNetDevice ();

  /**
   * \brief Add a 'port' to a bridge device
   * \param bridgePort the NetDevice to add
   *
   * This method adds a new bridge port to a PathHashingNetDevice, so that
   ...
   * bridging node you must enable IP on the PathHashingNetDevice itself,
...
private:
   ...
   */
  PathHashingNetDevice (const PathHashingNetDevice &);
   ...
   * \returns
   */
  PathHashingNetDevice &operator = (const PathHashingNetDevice &);
  ...
  Ptr<PathHashingChannel> m_channel; //!< virtual bridged channel
...
#endif /* PATH_HASHING_NET_DEVICE_H */
