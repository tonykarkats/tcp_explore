...
public:
  ...
  typedef enum
  {
    ...
    ECN_IDLE,			//< ECN is enabled but there is no action relevant to ECE or CWR to be taken
    ECN_CE_RCVD,		//< Receiver has received a packet with CE bit set in the IP header
    ECN_ECE_SENT,		//< Receiver has sent an ACK with ECE bit set in TCP header
    ECN_ECE_RCVD,		//< Sender has received an ACK with ECE bit set in TCP header
    ECN_CWR_SENT		//< Sende has reduced the congestion window and sent a packet with CWR bit set
  ...
  TracedValue<EcnState_t> m_ecnState;		  //!< Current ECN state, represented as combination of EcnState values
...
public:
  ...
  /**
   * \brief Callback function to hook to EcnState
   * \param oldValue old ecn state value
   * \param newValue new ecn ecn state valuee
   */
  ...
  /**
   * \brief Sets m_ecn to true for ECN functionality.
   *
   */
  void SetEcn();
...
protected:
  ...
  // Parameters related to ECN
  bool m_ecn;										// Socket ECN Capability
  TracedValue<SequenceNumber32> m_ecnEchoSeq;		// Seq number of the last received ECN Echo
  TracedValue<SequenceNumber32> m_ecnCESeq;			// Seq number of the last received CE
  TracedValue<SequenceNumber32> m_ecnCWRSeq;		// Seq number of the last sent CWR
