 ...
 */
#ifndef ECPM_NET_DEVICE_H
#define ECPM_NET_DEVICE_H

#include "ns3/net-device.h"
#include "ns3/mac48-address.h"
#include "ns3/nstime.h"
#include "ns3/ecmp-channel.h"
#include <stdint.h>
#include <string>
#include <map>
 ...
 *
 * The EcmpNetDevice object is a "virtual" netdevice that aggregates
 * multiple "real" netdevices and implements the data plane forwarding
 * part of IEEE 802.1D.  By adding a EcmpNetDevice to a Node, it
 ...
 * \brief a virtual net device that bridges multiple LAN segments
 */
class EcmpNetDevice : public NetDevice
{
public:
  ...
  EcmpNetDevice ();
  virtual ~EcmpNetDevice ();

  /**
   * \brief Add a 'port' to a bridge device
   * \param bridgePort the NetDevice to add
   *
   * This method adds a new bridge port to a EcmpNetDevice, so that
   ...
   * bridging node you must enable IP on the EcmpNetDevice itself,
  ...
  Ptr<NetDevice> hashToUplink(Ipv4Address src, Ipv4Address dst,uint16_t srcPort, uint16_t dstPort);
  uint32_t getHash(Ipv4Address src, Ipv4Address dst, uint16_t srcPort, uint16_t dstPort);

  // used in order for each switch not to produce the same exact hash
  uint32_t seed;
...
private:
   ...
   */
  EcmpNetDevice (const EcmpNetDevice &);
   ...
   * \returns
   */
  EcmpNetDevice &operator = (const EcmpNetDevice &);
  ...
  Ptr<EcmpChannel> m_channel; //!< virtual bridged channel
  ...
  // Flowlet specific attributes
  int m_ecmpMode;
  Time m_flowletGap;
  std::unordered_map<uint32_t, flowlet_t> flowlet_table;
