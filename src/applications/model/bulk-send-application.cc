  ...
  static TypeId tid = TypeId ("ns3::BulkSendApplication")
    ...
    .AddAttribute ("Remote", "The address of the destination",
     ...
     .AddAttribute ("Local", "The address of the source",
                    AddressValue (),
                    MakeAddressAccessor (&BulkSendApplication::m_source),
                    MakeAddressChecker ())
     .AddAttribute ("Filename", "The filename to store flow completion time",
                     StringValue (),
                     MakeStringAccessor (&BulkSendApplication::m_filename),
                     MakeStringChecker ())
  ...
  if (!m_socket)
    {
      ...
      m_socket->SetCloseCallbacks (
          MakeCallback (&BulkSendApplication::NormalCloseCallBack, this),
          MakeCallback (&BulkSendApplication::ErrorCloseCallBack, this));

    }

  this->originalBufferSize = m_socket->GetTxAvailable();
  NS_LOG_FUNCTION("Will send " << this->m_maxBytes <<
                  ", tcp buffer size is " << this->originalBufferSize <<
                  ", m_sendSize is " << m_sendSize);
...
void BulkSendApplication::ErrorCloseCallBack(Ptr<Socket> socket) {
  std::cout << "Connection experienced an error, maybe we reached maximum number of retries!\n";
  if (this->m_filename != "") {
    InetSocketAddress inetSourceAddr = Ipv4Address::ConvertFrom(this->m_source);
    InetSocketAddress inetDstAddr = InetSocketAddress::ConvertFrom(this->m_peer);

    std::ofstream resultFile;
    resultFile.open(this->m_filename, std::ios_base::app);
    resultFile << "Flow experienced data timeout, "
               << "From: "   << inetSourceAddr.GetIpv4()
               << ", To: "   << inetDstAddr.GetIpv4()
               << ":" << inetDstAddr.GetPort()
               << ", Size: " << this->m_maxBytes
               << "\n";
...
void BulkSendApplication::NormalCloseCallBack(Ptr<Socket> socket) {
	NS_LOG_FUNCTION(this);
}
...
void BulkSendApplication::SendData (void)
{
  ...
  while (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    { // Time to send more
      ...
      NS_LOG_FUNCTION ("sending packet at " << Simulator::Now ());
  ...
  // Check if time to close (all sent) and tcp buffer is empty
  // this means that all data have been sent and acknowledged
  if (m_totBytes == m_maxBytes && m_connected && (availableBuffer==this->originalBufferSize))
    {
      //uint32_t availableBuffer = m_socket->GetTxAvailable();
      //std::cout << "Available buffer : " << availableBuffer << "\n";
      m_socket->Close ();
      m_connected = false;

  	  if (this->m_filename != "") {
   			 Time ended = Simulator::Now();
   			 uint64_t duration = ended.GetNanoSeconds() - this->startTime.GetNanoSeconds();
   			 //std::cout << "Duration was " << duration << "\n";
   			 //std::cout << "Writing to " << this->m_filename << "\n";
   			 InetSocketAddress inetSourceAddr = Ipv4Address::ConvertFrom(this->m_source);
   			 InetSocketAddress inetDstAddr = InetSocketAddress::ConvertFrom(this->m_peer);

   			 std::ofstream resultFile;
   			 resultFile.open(this->m_filename, std::ios_base::app);
   			 resultFile << "From: "   << inetSourceAddr.GetIpv4()
   			            << ", To: "   << inetDstAddr.GetIpv4()
   			             << ":" << inetDstAddr.GetPort()
   			            << ", Size: " << this->m_maxBytes
   			            << ", Started in: " << this->startTime.GetNanoSeconds()
   			            << ", Ended in: " << ended.GetNanoSeconds()
   			            << ", Completed in " << duration << " Nano-seconds"
   			            << "\n";
...
void BulkSendApplication::ConnectionFailed (Ptr<Socket> socket)
{
  ...
  std::cout << "Connection failed, maybe we reached maximum number of SYN retries!\n";
  ...
  if (this->m_filename != "") {
    ...
    std::ofstream resultFile;
    resultFile.open(this->m_filename, std::ios_base::app);
    resultFile << "Flow experienced connection timeout, "
               << "From: "   << inetSourceAddr.GetIpv4()
               << ", To: "   << inetDstAddr.GetIpv4()
               << ":" << inetDstAddr.GetPort()
               << ", Size: " << this->m_maxBytes
